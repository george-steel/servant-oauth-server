{-# LANGUAGE OverloadedStrings, GADTs, FlexibleContexts, TypeApplications #-}
module Servant.OAuth.JWT (
    -- * Tokens
    FromJWT (..), ToJWT(..), ClaimSub(..),

    -- * Verification
    CompactJWT(..),
    SomeJWKResolver(..), JWTSettings,
    checkAuthToken,

    -- * Signing
    JWTSignSettings(..), makeAccessToken
) where
...
instance (FromHttpApiData a) => FromJWT (ClaimSub a) where
    fromJWT claims = fmap ClaimSub . parseQueryParam =<<
